	...
	def set_status(self, status):
		...
		for item in self.items:
			if item.prevdoc_docname:
				issue = item.prevdoc_docname
				break
		
		if issue:
			doc = frappe.get_doc("Issue", issue)
			if status == "Cancelled": doc.status = "Open"

			else: doc.status = status
			doc.save(ignore_permissions = True)
		
		frappe.db.set_value("Maintenance Order", self.name, "status", status)
